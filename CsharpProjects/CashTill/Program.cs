...
while (transactions > 0)
{
    ...
    try
    {
        ...
        System.Console.WriteLine("Transaction successfully completed.");
        registerCheckTillTotal += itemCost;
    }
    catch (InvalidOperationException e)
    {
        System.Console.WriteLine($"Could not complete transaction: {e.Message}");
    }
...
static void MakeChange(int cost, int[] cashTill, int twenties, int tens = 0, int fives = 0, int ones = 0)
{
    ...
    if (changeNeeded < 0)
        throw new InvalidOperationException("InvalidOperationException: Not enough money provied to complete the transaction.");
